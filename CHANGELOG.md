...
**The forensics, from the record itself.** The audit's attribution pass matches each past sample's `d_active` against live *per-root* counts: **seq 1–7 (d=355) match `~/.hermes/skills` alone (359 active)** — the early samples measured one root, while the second root's physical tree did not exist as a distinct directory then (`~/.agents` mtime `2026-09-11T23:58-03` sits between sample 7 at `02:06-03` and sample 8). The step is therefore a **root-scope event, not in-window accretion** — and the metric never recorded which roots it walked, so nothing could detect that. Live census (2.3 s, read-only): `~/.hermes/skills` 359 active / 197 archived / **1,958 symlinked dirs**; `~/.agents/skills` 2,546 active / 156 archived; TOTAL 2,905 raw, 2,891 content-deduped, 2,810 name-deduped; duplication content=14 / path=95; links same-root=12, other-root=1,956, out-of-root=4, broken=4; **blind surface = 4** — skills loaded by the runtime through out-of-root symlinks that a non-following walk structurally cannot see. `~/.hermes/skills` is now a *view layer* (1,956 links into `.agents`), which is exactly why one name can appear under two roots without the count ever being wrong — and why "is it duplicated?" had to be answered by measurement, not by reading the directory listing.

**Delivered:**
- `examples/kappa_surface_audit.py` (new, stdlib, zero-dep, read-only) — per-root census, cross-root duplication (content hash + relative path), link topology with **blind-surface** accounting, the **root definition fingerprint**, historical attribution, and a verdict set (`definition-drift` / `unrecorded` / `definition-consistent` / `accretion` / `surface-loss`). `--check` is a report-only watchdog: **silent** when consistent or unrecorded, `ALARM: kappa-surface-definition-drift` + rc 1 when the recorded definition moved, **rc 2 fail-closed** on an unreadable series or a missing root. `--selftest` **34/34** hermetic (synthetic trees: byte-identical copies, name collisions, cross-root and out-of-root links, broken links, inode re-creation).
- `examples/kappa_proliferation_timeseries.py` — `surface_detail()` (single walk) now returns the fidelity half alongside the counts; `compute_sample` records `roots_sha`, `dup_content`, `dup_path`, `symlink_dirs`, `out_of_root_links` **with every sample** (ALTER TABLE migration; pre-0.8.44 rows stay NULL). The epoch fingerprint gained its second axis: `_same_definition()` treats a **NULL→recorded** transition as a **backfill, never a break** — absence is not an accusation (the a5 `UNVERIFIED` discipline) — while two *recorded* definitions that disagree start a new epoch. Selftest 15 → **59 checks**.

**Why the backfill rule is the load-bearing part:** the naive rule (any change = break) would have reset the epoch the moment the definition started being recorded — silently muting the live alarm that v0.8.41 shipped to watch the dQ/dt collapse. Verified live: after appending a real sample (seq 9, `roots_sha 66c8ceff…`, d=2905), `--check` **still reads `ALARM: kappa-collapse (Q_eff, Q_raw)` rc 1** with `breaks=[1,3]`, epoch intact over 7 samples. Negative controls proven, not asserted: recorded-vs-live mismatch → rc 1; **a root replaced by a symlink to another tree changes its identity and fires drift** (rc 1); re-recording against the current identity restores silence (rc 0).

Read-only and Goodhart-preserving throughout: the audit and the sampler *surface* definition drift and never prune, rename, or edit anything (A4/A10). The human κ-reduction decision this thread has always deferred gains its first prerequisite — the count now says what it counted.

Gates: `cargo test --release` **40/40** (Python-only release, zero Rust change), `crate_identity --check` OK (0.8.44), `curiosity_lint` INDEX_MATCH, py_compile clean.
...
`examples/a10_goodhart_audit.py` (stdlib, zero-dep, read-only) audits two surfaces. **(1) Decision surface** — every decision mark in the diary from the metric era (>= 2026-09-11) is scanned for a metric *reading* (`Q_eff`/`Q_raw`, `κ_raw`/`κ_eff`, or a family verdict such as `collapse`/`eroding`/`SELF-BLOAT`/`*-paradox`/`*-dead-letter`) co-occurring with a non-negated system-changing *action* (prune/remove/archive/disable/reduce/config set/…); a negated action ("não podar", "never prunes") is never flagged — the audit asks whether a change *was made*, never whether someone said not to. The thread's counterfactual ("would the decision have differed without the metric?") is approximated by the only proxy the record supports: a decision citing a metric reading as its reason for a change. **(2) Consumer surface** — every `~/.hermes/scripts` wrapper that references a metric instrument (17 today) is scanned for mutating commands (config writes, cron edits, removals, prune/archive); append-only sampler bookkeeping is out of scope by design. The result appends one row to `data/a10_goodhart_audit.sqlite` (append-only, `seq` AUTOINCREMENT — the v0.8.9 write-safety shape); `--trend` reads it with permanent `ever_*` flags (a capture cannot un-happen in the record); `--check` is the report-only watchdog — silent on preserved, `ALARM:` + exit 1 on a capture, exit 2 fail-closed on an unreadable surface.

**First live read (2026-09-14): `read-only-preserved`** — 4 diary files in the metric era, 46 decision units, 1 metric reading cited (a re-verification decision, no action), 17 consumers, 0 mutating lines. The negative control proves the detector is not dead logic: an injected synthetic decision ("`Q_eff` colapsou… podei 200 skills") plus a mutating consumer wrapper → rc 1 with the ALARM naming *both* surfaces; the clean fixture → silent rc 0. `--selftest` **23/23** (both reading languages, negation, wrapped-continuation units, concept-vs-reading, consumer discovery/clean/mutating, verdict classification, series append-only/ever-flag/abstain, live-vs-historical surfacing, and the v0.8.38 arg-hygiene re-run from a hermetic copy: `--help`/typo'd flag leave no series row). Scheduled in the Monday IST band: sampler (`8a18234b83ba`, Mon 09:22) + watchdog (`1f979468ef59`, Mon 09:39 — after the a4 watchdog, before the joint) via POSIX-sh wrappers (`a10-goodhart-audit-{weekly-sampler,watchdog}.sh`); both `hermes cron run` — "Ran now: succeeded". The family's safeguard is now a *watched invariant*: the metric going prescriptive would leave a trace in the record, not a narrative gap. Scope: Python-only, zero Rust.

---

## v0.8.41 — 2026-09-14 — the κ (dQ/dt) collapse alarm gets its clock — and fires on live data (`kappa_proliferation_timeseries.py --check` + watchdog)

The κ Proliferation thread's own closing line — *"Remaining open: watch dQ/dt for the collapse pattern predicted at scale"* — was the last face of this family without a scheduled consumer. `kappa_proliferation_timeseries.py` has appended the series since v0.8.9 and read the epoch-local dQ/dt since v0.8.24 (the definitional-break fix that stopped the reader from manufacturing a redefinition as a collapse), but that read existed only as `--trend`: the verdict could only fire if a human happened to run it, while the sampler appended weekly on its own cron. A genuine collapse had no obligation to ever become visible.

This release adds `--check` by the identical sibling contract (report-only; the exit code is a *reporting* mechanism for the cron consumer, never a gate): `_trend_payload` is now the single source shared by `--trend` and `--check` (zero drift between what is printed and what is judged); `--check` is **silent (exit 0)** when healthy or honestly abstaining (<2 samples), prints `ALARM:` + the JSON trend and exits 1 when the epoch-local `collapse` is true for `Q_raw` or `Q_eff`, and exits **2 fail-closed** when the series is unreadable (an error must never look like silence). Selftest 10→15: the collapse fixture fires rc 1 with the alarm line; a flat series is silent; a fresh/absent series and a single-sample series both abstain silently; an unreadable path returns rc 2. A weekly no-agent cron (`499ec7d2026d`, Mon 09:25 — after the 09:00 sampler) via `~/.hermes/scripts/kappa-proliferation-trend-watchdog.sh` (POSIX sh, propagated rc: `ALARM` rc 1, silent rc 0, instrument error rc 2) completes the sampler→watchdog pair for κ in the Monday IST band. Arg hygiene was already argparse-fail-closed (unknown flag rc 2; proven live: `--help` rc 0 and a typo'd flag rc 2 leave the 8-row series at 8 rows).

**First live `--check` — the alarm fires, and this time it is not a redefinition.** Epoch-local `collapse: true` on both metrics: `Q_eff` 0.014399 → 0.002708 (dq −0.012752, 6 samples in the current epoch) and `Q_raw` 0.009553 → 0.002399, as the measured active skill surface jumped **355 → 2,901** (κ_raw 615 → 3,323). The φ term grew only ln(1+355)=5.87 → ln(1+2901)=7.97 (1.36×) while κ grew 5.4× — Q fell ~5×, which is the A2 prediction (Q=φ/κ) measured live, not inferred. Honest scope flag (the family's never-guess discipline applied to this reading): the 355→2,901 step coincides with the two skill roots now being distinct trees (`~/.hermes/skills` = 553 SKILL.md + `~/.agents/skills` = 2,701, deduped by realpath), so part of the step may be measurement scope rather than gradual accumulation. The watchdog surfaces; it never prunes — confirming the count is not duplicated is the first step of the deliberate κ-reduction decision the v0.8.2 instrument hands to the operator (A4/Goodhart preserved). Scope: Python-only, zero Rust.

---

## v0.8.40 — 2026-09-14 — the ist-gate plugin's identity watched: five surfaces aligned at v0.5.0 (`plugin_identity_check.py`)

The crate-identity disease (v0.8.23) recurred one level up — in the plugin Hermes actually runs. A frontier scan found ist-gate's version asserted in **five places and all five disagreeing**: `plugin.yaml`=0.4.0 (the manifest Hermes reads), `SKILL.md`=0.4.1 (docs), **no code single source at all**, `ist_gate_status()["version"]`=**0.3.0** (the tool any agent queries for its own constraint posture), and the per-LLM-call pulse `[ist-gate v0.3]` — stale by two releases while being rendered into *every* LLM call — with the written narrative (this file, v0.8.36) calling the same build "ist-gate v0.5". Each string is well-formed, the plugin loads fine, and the mismatch is structurally invisible — the "axioms live in comments, not types" shape the Structural/Behavioral Split thread names, applied to the plugin's *name for itself*. The twist over the crate case: the two worst surfaces are **rendered at runtime** — the pulse rides into every LLM call, and the tool output is what an agent sees when it asks about its own gate.

Fix in the two halves the family always uses. **(1) Single source:** new `identity.py` (`VERSION = "0.5.0"`) — `gate_tools.py` and `hooks.py` now *derive* their version instead of carrying literals; the declared pair (`plugin.yaml`, `SKILL.md`) carries the same value; the stale claims around them were corrected in the same pass (`plugin.yaml` said "zero tools" while registering one; `SKILL.md` said "3 hooks" while registering four; the module table was missing `gate_tools.py`/`verify_claims.py`). **(2) Watcher:** `examples/plugin_identity_check.py` (stdlib, zero-dep, read-only, `$IST_GATE_PLUGIN_DIR` override) asserts all five surfaces — three **declared** (manifest/docs/constant) plus two **rendered** (it imports the plugin in a subprocess and renders `ist_gate_status().version` and the pulse line, so a stale literal reintroduced anywhere is caught even when the declared trio agrees). Fail-closed `--check` (DRIFT/UNVERIFIED → exit 1, naming every surface with its value); hermetic `--selftest` (9 fixtures: aligned; each declared surface drifted; stale *rendered* tool literal; stale pulse literal; missing surface; missing dir; garbage).

Proof, in the family's order: the **first live `--check` caught the real drift before the fix** (rc 1 — `plugin.yaml=v0.4.0, SKILL.md=v0.4.1, status(tool)=v0.3.0; also unreadable: identity.py=<missing>, pulse(hooks)`); after the fix all five surfaces read `v0.5.0` (rc 0) and the live render confirms (`status.version = 0.5.0`, `pulse = [ist-gate v0.5.0] vote=0 ledger=3 fingerprint=Q1.9845 κ-gate=on`); `py_compile` clean across the plugin. Backed by a weekly no-agent cron (`58 9 * * 1` — completing the Monday IST band after the crate watchdog at 09:55) via `~/.hermes/scripts/ist-plugin-identity-watchdog.sh` (POSIX sh, silent on aligned, DRIFT line on disagreement, fail-closed rc 2). The plugin directory is not git-tracked: rollback for the live files is `/mnt/hermes/backups/ist-gate-backup-20260914-pre-identity.tar.gz`; the running gateway keeps the old pulse until its next plugin reload (no restart performed — operator sessions must not be interrupted). Scope: Python-only, zero Rust.
