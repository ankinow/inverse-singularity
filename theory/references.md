...
| **DeepMind 2026 — Multi-Agent Error Amplification** | Cited in the 2026-08-14 frontier podcast "O dilema da IA que se reescreve" (~9:30). Unstructured multi-agent environments amplify errors up to **17×**. The empirical anchor for `system_kappa = local + children·coord_kappa + merge_kappa`: coordination cost > quality gain ⇒ the Gateway refuses (QualityCrossover). The 17× pattern is bounded by design. |
| **LERMF post-mortem (v3.0 → v4.0, 2026)** | Cited in the same podcast (~12:30) as the cautionary tale of IST reduced to "bloat-to-version". Agent shrunk the font instead of distilling meaning. v4.0 fix: separate mechanical-economy from strategy-generation. The v0.7.0 Delegation Gateway is this separation in runtime form. |
| **Frontier podcast — "O dilema da IA que se reescreve"** (2026-08-14, pt-BR, 23:26) | Primary 2026 frontier anchor for IST. 20 canonical concepts (Hermes, SOUL.md, IST, RSIP, Karpathy Auto-Research Loop, AutoDream, EvalDim9D, MCP, YOLO/Smart Approvals, AGP, Ponte-Feliz, Quantum Library, LLMLingua4, Spotlighting, DeepMind 17×, etc.). Mapped to runtime in `wiki/the-dilemma-of-self-rewriting-ai-2026.md`. |
